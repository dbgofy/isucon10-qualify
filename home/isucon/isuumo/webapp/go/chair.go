...
func postChair(c echo.Context) error {
	...
	for _, row := range records {
		...
		_, err = tx.Exec("SET @height = ?; SET @width = ?; SET @depth = ?;INSERT INTO chair_geom(id, g) VALUES(?, ST_GEOMFROMTEXT( CONCAT('POLYGON ((', LEAST(@height, @width, @depth), ' ', (@height + @width + @depth - LEAST(@height, @width, @depth) - GREATEST(@height, @width, @depth)), ', ', LEAST(@height, @width, @depth), ' 1000, ' '1000 1000, ', '1000 ', (@height + @width + @depth - LEAST(@height, @width, @depth) - GREATEST(@height, @width, @depth)), ', ', LEAST(@height, @width, @depth), ' ', (@height + @width + @depth - LEAST(@height, @width, @depth) - GREATEST(@height, @width, @depth)), '))'))", height, width, depth)
		if err != nil {
			c.Logger().Errorf("failed to insert chair: %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
...
func searchChairs(c echo.Context) error {
	...
	conditions = append(conditions, "stock_flag = TRUE")
...
func buyChair(c echo.Context) error {
	...
	var chair Chair
	err = tx.QueryRowx("SELECT id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock FROM chair WHERE id = ? AND stock_flag = TRUE FOR UPDATE", id).StructScan(&chair)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	_, err = tx.Exec("UPDATE chair SET stock = ?, stock_flag = ? > 0 WHERE id = ?", chair.Stock-1, chair.Stock-1, id)
	if err != nil {
...
func getLowPricedChair(c echo.Context) error {
	var chairs []Chair
	query := `SELECT id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock FROM chair WHERE stock_flag = TRUE ORDER BY price ASC, id ASC LIMIT ?`
