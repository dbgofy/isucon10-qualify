...
func initialize(c echo.Context) error {
	...
	paths := []string{
		...
		filepath.Join(sqlDir, "5_DummyDoorGeometryData.sql"),
...
func postEstate(c echo.Context) error {
	...
	for _, row := range records {
		...
		_, err = tx.Exec("INSERT INTO door_geom(id, g) VALUES(?,POINT(?,?))", id, min, max)
		if err != nil {
			c.Logger().Errorf("failed to insert estate: %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	var estates []Estate
	query = `SET @chair_geom = (SELECT ST_ASTEXT(g) FROM chair_geom WHERE id = ?);SELECT id, thumbnail, name, description, latitude, longitude, address, rent, door_height, door_width, features, popularity FROM estate JOIN door_geom USING (id) WHERE ~ST_CONTAINS(@chair_geom, g) ORDER BY popularity DESC, id ASC LIMIT ?`
